"""`multivarka refine <task>` — run another round on top of previous output.
...
def _run_one(cook_dir: Path, project: str, participant: dict,
    ...
    print(f"[refine] {name} ({flavor}): launching service {service}", flush=True)
    try:
...
def refine(name: str, root: Path,
    ...
    print(f"[refine] next: multivarka judge {name} --docker")
