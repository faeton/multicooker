...
their writable worktree. They get:
  - BRIEF.md  (read-only symlink to ../../BRIEF.md)
  - raw/      (read-only symlink to ../../raw/)
...
def cook(name: str, root: Path, use_docker: bool = False,
    ...
    if use_docker:
        print("error: --docker mode not implemented in v0.1; use host-mode "
              "(omit --docker). Dockerfiles in templates/cook/participants/ "
              "are scaffolds for future work.", flush=True)
        return 2
