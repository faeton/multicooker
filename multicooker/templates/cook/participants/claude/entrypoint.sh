#!/bin/sh
# claude entrypoint inside multicooker sandbox.
...
# If MULTICOOKER_MODEL is set, pass --model so brief.yaml can pin the model
...
if [ -n "$MULTICOOKER_MODEL" ]; then
  exec claude --model "$MULTICOOKER_MODEL" --print --dangerously-skip-permissions "$PROMPT" --add-dir /work
fi
