...
in `mc-base-<flavor>:latest`, built once. Per-cook images derive from these
with `FROM mc-base-<flavor>:latest` and only layer in the entrypoint, so
...
def image_tag(flavor: str) -> str:
    return f"mc-base-{flavor}:latest"
...
def is_built(flavor: str) -> bool:
    """True iff `docker image inspect mc-base-<flavor>:latest` succeeds."""
    res = subprocess.run(
...
def build(flavor: str, force: bool = False) -> None:
    """Build mc-base-<flavor>:latest from templates/base/<flavor>/."""
