"""`multicooker add-participant <task> <spec>` — add a new participant to a cook.

Spec is the same shape as `multicooker new --participants`:
...
def add_participant(name: str, root: Path, spec: str) -> int:
    ...
    print(f"next: multicooker doctor {name} && multicooker cook {name}")
    return 0
