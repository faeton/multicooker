...
в `multivarka/host_runner.py:_RL_PATTERNS`, наследие из arena.
...
Resume — отдельный flow (`multivarka resume <task>`, в TODO для
v0.2).
...
reproxy-arena: 4.3 GB за две ночи. У мультиварки артефакт = только
...
`multivarka/host_runner.py:run_all`.
...
prompt, а не в файл.** В мультиварке prompt контейнера = "Read
