...
поведение. Если `multivarka cook --docker` падает с "not
...
- Сети `clients-<task>` (internal) + `llm-egress` (allowlist).
...
| компонент                              | состояние                                              |
|----------------------------------------|--------------------------------------------------------|
| `multivarka new <task>`                | ✅ работает, копирует `templates/cook/`                 |
...
| `multivarka cook <task>` host-mode     | ✅ работает (`host_runner.py`) — это временный фоллбек  |
| `multivarka cook <task> --docker`      | ❌ `error: --docker mode not implemented in v0.1`       |
| `multivarka judge <task>`              | ✅ host-mode; копирует (не симлинкует) — это правильно  |
| `multivarka judge <task> --docker`     | ❌ нет                                                  |
| Подписочная auth в контейнерах         | ❌ не подключено                                        |
| compose.yaml per cook                  | ❌ не генерируется                                      |
| Сети `clients-<task>` / `llm-egress`   | ❌ нет                                                  |
| Allowlist egress proxy                 | ❌ нет (опционально, см. ниже)                          |

## Что нужно сделать, чтобы привести код к CLAUDE.md

### 1. Auth setup (`multivarka init-auth`)

Новая команда. См. `docs/auth.md` — она проверяет/готовит:
- `~/.codex/auth.json` присутствует;
- `~/.gemini/oauth_creds.json` присутствует;
- собрать `mv-claude-base` Docker image;
- запустить интерактивный `claude /login` в контейнере с named
  volume `mv-claude-auth`;
- echo-test всех трёх.
...
- сети `clients` (internal) + `egress` (bridge);
...
def run_cell(cook_dir, participant_name, flavor, timeout_s) -> CellResult:
    # docker compose -p mv-<task> up -d participant-<name>
    # docker logs --follow → парсим rate-limit signatures
    # wait timeout / exit
    # docker compose -p mv-<task> rm -fv participant-<name>
    ...
```
