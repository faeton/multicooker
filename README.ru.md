...
Запускает несколько LLM-агентов (`claude`, `codex`, `agy`,
`grok`) параллельно решать **одну и ту же задачу** — каждого в
своём docker-контейнере, со своей подписочной авторизацией — а
потом другие LLM-агенты читают результаты вслепую (под метками
`A`/`B`/`C`), выставляют оценки по твоей рубрике и пишут review.

На выходе — `leaderboard.md` + корпус из N расходящихся решений
одной задачи. **Без счетов за API**: ходит через твой `Claude Pro`
/ `ChatGPT Plus` / `Gemini Advanced` / `SuperGrok`.
...
- Хотя бы одна из CLI: `claude` (`claude /login`), `codex`
  (`codex` для входа), `agy` (переиспользует твой OAuth из
  `~/.gemini/oauth_creds.json`), `grok` (`grok login`). Только
  те flavor'ы, которые реально хочешь гонять.
...
codex / agy), который уже объясняет любому агенту устройство
...
claude        # или: codex, или: agy — все читают AGENTS.md
```

Дальше просто опиши задачу словами:

> *«Собери cook `landing-redesign`. Сравни claude / codex / agy
...
```bash
# Запуск design-примера (нужны claude/codex/agy логины; grok опционально)
